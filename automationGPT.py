...
class ChatGPT:
    ...
    def __init_browser(self) -> None:
        ...
        import requests
        # res = requests.get(
        #     'http://local.adspower.net:50325/api/v1/browser/start?user_id=&launch_args=["https://nowsecure.nl"]'
        # ).json()
    ...
    def __ensure_cf(self, retry: int = 3) -> None:
        ...
        for _ in range(retry):
            ...
            try:
                ...
                self.driver.close()
                self.driver.switch_to.window(original_window)
                return  # Successfully passed the challenge

        raise ValueError('Cloudflare challenge failed')  # Failed after retry attempts
    ...
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        textbox = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(chatgpt_textbox))
        self.driver.execute_script('arguments[0].value = arguments[1]; arguments[0].dispatchEvent(new Event("change"));', textbox, message)
        ...
        WebDriverWait(self.driver, 300).until_not(EC.presence_of_element_located(chatgpt_streaming))

        self.logger.debug('Getting response...')
        responses = WebDriverWait(self.driver, 10).until(EC.presence_of_all_elements_located(chatgpt_big_response))
        if responses:
            response = responses[-1]
            if 'text-red' in response.get_attribute('class'):
                self.logger.debug('Response is an error')
        ...
        if not matches:
            ...
            try:
                WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(chatgpt_chats_list_first_node)).click()
            except Exception as e:
                ...
            time.sleep(5)
            matches = pattern.search(self.driver.current_url)
    ...
    def refresh_chat_page(self) -> None:
        ...
        '''
        if not self.driver.current_url.startswith(chatgpt_chat_url):
            self.logger.debug('Current URL is not chat page, skipping refresh')
            return

        self.driver.get(f'{chatgpt_chat_url}/?model={self.__model if self.__model == "4" else ""}')
    ...
    def close_chat_research_popup(self) -> None:
        ...
        self.driver.refresh()  # Refresh the page before interacting with the elements
        WebDriverWait(self.driver, 30).until(EC.visibility_of_element_located((By.TAG_NAME, "body")))  # Wait for the page to load
        ...
        try:
            ...
            print("Not enough neutral buttons found.")

        time.sleep(random.uniform(1, 3))

        try:
            button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button.btn-primary")))
            buttons = self.driver.find_elements(By.CSS_SELECTOR, "button.btn-primary")
            if len(buttons) > 1:
                buttons[1].click()
        except IndexError:
            print("Not enough primary buttons found.")
